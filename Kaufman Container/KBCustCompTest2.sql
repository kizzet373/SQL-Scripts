...
(
	...
	(DENSE_RANK() OVER(ORDER BY Calculated_Sales ASC)) AS Calculated_RankNumber,
...
(
	...
	SELECT
		Customer_SalesRepCode,
		SalesRep_Name,
		NULL, --Customer_CustID--
		'TOTAL', --Customer_Name--
		NULL, --Calculated_RankNumber--
		'', --Customer_Industry--
		SUM(Calculated_DollMonth) AS Calculated_DollMonth,
		SUM(Calculated_MonthGPDoll) AS Calculated_MonthGPDoll,
		(CAST(CAST((CASE WHEN SUM(Calculated_FrtMth) <> 0 THEN (CASE WHEN SUM(Calculated_DollMonth) = 0 THEN 0 ELSE  ( SUM(Calculated_DollMonth) - SUM(Calculated_NewCostMonth) - SUM(Calculated_FrtMth) )  / SUM(Calculated_DollMonth)  * 100   END) ELSE (CASE WHEN SUM(Calculated_DollMonth) = 0 THEN 0 ELSE  ( SUM(Calculated_DollMonth) - SUM(Calculated_NewCostMonth)  )  / SUM(Calculated_DollMonth)  * 100   END) END)as DECIMAL(18,1)) AS NVARCHAR(20)) + '%')
		AS Calculated_MTDGPPer,
		SUM(Calculated_Sales) AS Calculated_Sales,
		SUM(Calculated_TotGPDoll) AS Calculated_TotGPDoll,
		(CAST(CAST(CASE 
 		WHEN SUM(Calculated_TotalFreight) <> 0 THEN(
 			CASE 
 				WHEN SUM(Calculated_Sales) = 0 THEN 0 
 				ELSE  ( SUM(Calculated_Sales) - SUM(Calculated_NewCost) - SUM(Calculated_TotalFreight))  / SUM(Calculated_Sales)  * 100   
 			END) 
 		ELSE(
 			CASE 
 				WHEN SUM(Calculated_Sales) = 0 THEN 0 
 				ELSE  ( SUM(Calculated_Sales) - SUM(Calculated_NewCost)  )  / SUM(Calculated_Sales)  * 100   END) 
			END AS DECIMAL(18,1)) AS NVARCHAR(20)) + '%') AS Calculated_TotGPPer,
		SUM(Calculated_PrevSales),
		SUM(Calculated_PrevGPDoll),
		(CASE  
			WHEN SUM(Calculated_PrevCost) = 0 or SUM(Calculated_PrevCost) = null THEN null
			WHEN SUM(Calculated_PrevSales) = 0 or SUM(Calculated_PrevSales) = null THEN null 
			ELSE (CAST(CAST((SUM(Calculated_PrevCost) / SUM(Calculated_PrevSales) * 100.0) AS DECIMAL(18,1)) AS NVARCHAR(20)) + '%')
			END) AS Calculated_PrevGPPer,
		(SUM(Calculated_Sales) - SUM(Calculated_PrevSales)) AS Calculated_YTDDifference,
		(CASE  
			WHEN SUM(Calculated_PrevSales) = 0 or SUM(Calculated_PrevSales) = null THEN null
			ELSE CAST(CAST(((SUM(Calculated_Sales) - SUM(Calculated_PrevSales)) / SUM(Calculated_PrevSales) * 100) AS DECIMAL(18,1)) AS NVARCHAR(20))+ '%'
		END) AS Calculated_YTDDifferencePer
		FROM Sales_CTE
	GROUP BY SalesRep_Name, Customer_SalesRepCode
	UNION ALL
	SELECT
		NULL, --Customer_SalesRepCode--
		'GRAND TOTAL', --SalesRep_Name--
		NULL, --Customer_CustID--
		'GRAND TOTAL', --Customer_Name--
		NULL, --Calculated_RankNumber--
		'', --Customer_Industry--
		SUM(Calculated_DollMonth) AS Calculated_DollMonth,
		SUM(Calculated_MonthGPDoll) AS Calculated_MonthGPDoll,
...
ORDER BY (CASE SalesRep_Name WHEN 'GRAND TOTAL' THEN '1' ELSE '0' END), SalesRep_Name , (CASE Customer_Name WHEN 'TOTAL' THEN '1' ELSE '0' END), Calculated_Sales