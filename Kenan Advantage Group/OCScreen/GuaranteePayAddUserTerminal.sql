--GuaranteePay User Terminals Query.
...
DECLARE @FirstName VARCHAR(10) = 'Kirk',
        ...
        @Terminals VARCHAR(MAX) = '51097';   --Supports single or comma-delimited terminals   '53027'   or   '53027,53027,53027'
...
FROM dbo.labelfile lf WITH
    (NOLOCK)
WHERE lf.labeldefinition = 'TERMINAL'
      AND lf.retired = 'N'
      AND lf.abbr IN
          (
              SELECT value FROM dbo.CSVStringsToTable_fn(@Terminals)
          )
      AND lf.abbr NOT IN
          (
              SELECT Terminal
